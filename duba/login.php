...
<body style="background-color: rgba(17, 24, 39, 1);">
		...
		<div class="form-container-login">
			...
			<form class="form" action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
				...
				<div class="input-group">
					...
					<div class="forgot">
						<a rel="noopener noreferrer" href="forgotpassword.html" class="">Şifremi Unuttum.</a>
					</div>
				</div>
				<button class="sign">Giriş yap</button>
			</form>
			<br>
			<p class="signup">Hesabınız yok mu?
				<a rel="noopener noreferrer" href="register.php" class="">Kayıt ol</a>
			</p>
		</div>
	</div>
	<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (mysqli_num_rows($result) > 0) {
			...
			$sql = "SELECT * FROM kullanicilar WHERE kullanici_adi = ? AND sifre_hash = ?";
